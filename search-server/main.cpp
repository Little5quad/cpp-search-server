...
void TestRelevanceSort() {
    ...
    {//Сортировка по убыванию релевантности, подсчёт рейтинга, относительное сравнение релевантности - порог +-0.001
        SearchServer server;
...
void TestSearchStatusDocs() {
    ...
    // Сначала убеждаемся, что поиск документа даёт результат с двумя строками
    {
        ...
        {//Ищем актуальные (Вообще должно совпадать с прошлыми вызовами без аргумента, но хитрые пользователи могут сломать и это)
            ...
            ASSERT_EQUAL(doc0.id, doc_id_4);
            ASSERT_EQUAL(doc0.rating, 1);
            ASSERT(isRelativelyEqual(doc0.relevance, 0.6668070516454887, 1e-1));
            ASSERT_EQUAL(doc1.id, doc_id_1);
            ASSERT_EQUAL(doc1.rating, 0);
            ASSERT(isRelativelyEqual(doc1.relevance, 0.34657359027997264, 1e-1));
        }
        {//Ищем забаненные
            ...
            ASSERT_EQUAL(doc0.id, doc_id_2);
            ASSERT_EQUAL(doc0.rating, 1);
            ASSERT(isRelativelyEqual(doc0.relevance, 0.03646431135879092, 1e-1));
        }
        {//Ищем нерелевантные
            ...
            ASSERT_EQUAL(doc0.id, doc_id_3);
            ASSERT_EQUAL(doc0.rating, 1);
            ASSERT(isRelativelyEqual(doc0.relevance, 0.13222378676848182, 1e-1));
            ASSERT_EQUAL(doc1.id, doc_id_6);
            ASSERT_EQUAL(doc1.rating, 7);
            ASSERT(isRelativelyEqual(doc1.relevance, 0.013022968342425327, 1e-1));
        }
        {//Ищем удалённые
            ...
            ASSERT_EQUAL(doc0.rating, 2);
            ASSERT(isRelativelyEqual(doc0.relevance, 0.045580389198488648, 1e-1));
        }
    }
}
